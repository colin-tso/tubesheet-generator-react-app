...
const unchangedAtDisplayPrecision = (prev, next) => utils.isNumber(prev) && prev > 0 && utils.trunc(prev, 2) === utils.trunc(next, 2);
function fieldsReducer(state, action) {
    switch (action.type) {
        ...
        case "SET_TUBE_CLEARANCE": {
            ...
            const pitchRatio = utils.isNumber(state.tubeOD) && state.tubeOD > 0
...
export function useLayoutForm({ lastSingleResult, postCalculateSingle, postCalculateAll, }) {
    const [fields, dispatch] = useReducer(fieldsReducer, initialFieldValues);
    const [layoutInputsDefined, setLayoutInputsDefined] = useState(false);
    const [layoutOptionSelected, setLayoutOptionSelected] = useState(false);
    const setGenericField = useCallback((name, value) => {
    ...
    const onAcceptEmpty = (value, name) => {
        if (value.trim() === "") {
            setGenericField(name, undefined);
        }
    };
    const requestAllLayoutResults = useCallback((overrides) => {
        ...
        const valid = utils.isNumber(effOTLtoShell) &&
    ...
    const inputOnSubmitHandler = (e) => {
        e.preventDefault();
    };
    // Helper to package and send single-layout calculation requests.
    const triggerSingleCalculation = useCallback((overrides) => {
        const effOTLtoShell = withOverride(overrides, "OTLtoShell", fields.OTLtoShell);
        const effTubeOD = withOverride(overrides, "tubeOD", fields.tubeOD);
        const effPitchRatio = withOverride(overrides, "pitchRatio", fields.pitchRatio);
        const effMinTubes = withOverride(overrides, "minTubes", fields.minTubes);
    ...
    const onKeyDown = (e) => {
        ...
        const inputsValid = utils.isNumber(next.OTLtoShell) &&
    ...
    const validateLayoutInputs = useCallback(() => {
        const valid = utils.isNumber(fields.OTLtoShell) &&
            utils.isNumber(fields.tubeOD) &&
    ...
    useEffect(() => {
        if ((lastSingleResult === null || lastSingleResult === void 0 ? void 0 : lastSingleResult.shellID) && (lastSingleResult === null || lastSingleResult === void 0 ? void 0 : lastSingleResult.numTubes)) {
            dispatch({
                type: "SET_FIELD",
                field: "actualTubes",
        ...
        else if (lastSingleResult === null || lastSingleResult === void 0 ? void 0 : lastSingleResult.numTubes) {
            dispatch({
                type: "SET_FIELD",
                field: "actualTubes",
                value: undefined,
